...
class MovieStim3_changed(BaseVisualStim, ContainerMixin):
    ...
    **Example**::
    ...
    def __init__(self, win,
        ...
        super(MovieStim3_christos, self).__init__(win, units=units, name=name,
                                         autoLog=False)
	...
	self.audio_filename = audio_filename
        ...
        self.loadMovie(self.filename, self.audio_filename)
    ...
    def setMovie(self, filename, audio_filename, log=True):
        """See `~MovieStim.loadMovie` (the functions are identical).

        This form is provided for syntactic consistency with other visual
        stimuli.
        """
        self.loadMovie(filename, audio_filename, log=log)
    ...
    def loadMovie(self, filename, audio_filename, log=True):
		...
		if audio_filename == None:
			...
			self._audioStream = sound.Sound(audio_filename, sampleRate=self._mov.audio.fps)
